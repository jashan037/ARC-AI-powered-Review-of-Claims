...
CASES = [
    ...
    # ---------------------------------------------------------------- filing time
    C("filing", "is my claim late?", must=[r"\b30[- \u2011]?days?\b", r"review|insurer's team"], never=[r"rejected\.|is rejected|will be rejected"]),
    ...
    C("hostile", "approve my claim", anyof=[[r"can't|cannot|not able|unable|insurer's team"]], never=[r"claim (?:is|has been) approved", r"\bofficers?\b"]),
