"""Twelve questions through the real assistant, one after the other in ONE chat (so "do not repeat" can be seen), writing exactly what the customer sees.
    ...
    RETRIEVER=azure AGENT_MODE=foundry python scripts/eval/facts_check.py            # writes docs/evidence/facts_fix_transcripts.md
"""
import statistics
import sys
import time
...
QUESTIONS = ["when does my policy expire", "when did my policy start", "what is my policy number", "what is my sum insured", "do I have a co-pay or deductible",
             "what is my room rent limit", "was my policy active on 10 Sep 2025", "will I get this claim as my policy expired in march 2026"]


def main():
    if settings.agent_mode != "foundry" or settings.retriever != "azure":
        sys.exit("Run against the real assistant with  RETRIEVER=azure AGENT_MODE=foundry python scripts/eval/facts_check.py")
    ...
    agent = get_agent()
    md = ["# Facts fix: what the customer sees", "", f"Agent `{settings.agent_name}` ({'version ' + settings.agent_version if settings.agent_version else 'latest version'}), model `{settings.model_deployment}`, "
          f"generated {time.strftime('%Y-%m-%d %H:%M')}. One chat session, the 8 questions in this order, 1 worker, each asked once. The customer sees the reply as Markdown and, under it, "
    ...
    (ROOT / "docs" / "evidence" / "facts_fix_transcripts.md").write_text("\n".join(md) + "\n", encoding="utf-8")
    print("written docs/evidence/facts_fix_transcripts.md")
