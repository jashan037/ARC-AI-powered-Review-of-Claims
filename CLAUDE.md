...
Served by FastAPI. **Three views at three real paths, one HTML file** (redesigned 22 Sep 2026: white UI, black ink, one cyan accent `#38C6EC`): `/` (one-screen landing: a black pill nav with a burger menu below 940px, a two-line headline, "Upload your documents" + a sample link, the demo note, a CSS-animated cyan band and a product picture drawn in markup; on desktop a 1290x860 stage scaled to the window by `fit()` in `app.js`, below 940px a CSS grid; a one-time entrance armed by `static/enter.js` in `<head>`; below the hero a second screen `#metrics`, "Built for Intelligent Claim Review": LED-dot type drawn by `dots()` in `app.js`, three glass cards scaled as rigid 429x554 units with real figures (8 answer checks, 186 clauses, 68 Annexure B items) and a "Learn more" panel each, entrance played once on scroll), `/upload` (heading, one drop card, file rows with status pills, a sample link, and a macOS-style glass loading card `#loader` while files are read), `/chat` (ARC's replies with a CSS-drawn mark, tinted customer bubbles, three starting-question chips under the first message, a fixed composer with an add-documents button and a send button whose cyan ring spins while ARC works; top bar with "Start over" + "Download report" once the claim is ready). History API, `sessionStorage` for the session id, `/chat` without a ready claim redirects to `/upload`, and a refresh restores the conversation from `GET /sessions/{id}/messages`. Every UI string is in one `TEXT` block in `app.js`. Static files in `app/static` (`index.html`, `app.css` with the design tokens at the top, `app.js`, `enter.js`, `md.js`, `logo.svg`, `fonts/InterVariable.woff2` + OFL). No build step, no CDN, no external request; strict CSP (no inline script or style, `font-src 'self'`), so no Google Fonts and no remote video. The logo is a black rounded tile with a cyan arch over a white dot. Presentation material: `docs/TEASER.md` (20 s storyboard), `docs/KEYNOTE.md` (3-4 min script), `docs/architecture.html` (glass diagram for recording). Screenshots: `python scripts/dev/ui_screenshots.py` -> `docs/screenshots/final/`. Browser tests incl. axe-core: `tests/test_ui.py`.
...
- Keep the tests green (**555**, all offline; the markdown tests need Node, the browser tests need Playwright with the installed Chrome, and both skip themselves without them).
...
python -m pytest tests -q                    # 555 pass
