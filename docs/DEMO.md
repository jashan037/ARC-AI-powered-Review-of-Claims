...
**Verified** on 20 Sep 2026 against the real Azure agent (`claims-adjudication-agent-v2`, version 4, gpt-5-mini, index `claims-kb-v2`): all 8 steps below passed in two consecutive runs of `scripts/demo_check.py` after the last change (121 s and 143 s of total agent time; 10 to 25 seconds per answer), so talk while it works.
...
|---|---|---|---|---|
| 1 | A | `Is knee replacement covered and what is the waiting period?` | **Covered, with conditions.** The first point says the policy explicitly lists **"Joint replacement surgeries"** among the specified surgical procedures. Then the 24-month rule, the **accident exception**, and the 30-day initial period. "What to check next" asks the officer to verify inception date, accident, and any sum-insured enhancement. | **24 months** (Excl02), 30 days (Excl03), accidents exempt. Cites **C.1.b.vi first**, then C.1.b and C.1.c, each with its page. The Evidence quote for C.1.b.vi is centred on the entry: "...Hydrocele/Rectocele - **Joint replacement surgeries** - Surgery for nasal septum deviation...". |
...
- **Wording varies between runs.** In the last two full evaluation passes (168 runs) two runs drifted on a policy question: one left out the accident exception (knee replacement), and one typed a question the wording does answer as "Insufficient information" because the exact dates were missing (Q02, the accident-within-30-days question, which is not one of the 8 steps). The 8 demo steps themselves passed every time. Transcripts are in `examples/eval_failures/`.
