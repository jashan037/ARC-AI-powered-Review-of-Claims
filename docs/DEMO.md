...
**Verified** on 20 Sep 2026 against the real Azure agent (`claims-adjudication-agent-v2`, version 3, gpt-5-mini, index `claims-kb-v2`): all 8 steps below passed twice in a row through `scripts/demo_check.py`. The agent needs 11 to 25 seconds per answer (about 2.5 minutes over all 8 steps), so talk while it works.
...
|---|---|---|---|---|
| 1 | A | `Is knee replacement covered and what is the waiting period?` | **Covered, with conditions.** A 24-month specified-procedure waiting period, not applicable after an accident, possibly 36 months if the knee disease is pre-existing, plus the 30-day initial period. | **24 months** (Excl02), **36 months** (Excl01), 30 days (Excl03). Cites C.1.b, C.1.a, C.1.c, each with its page. |
...
- **Step 1** may justify "knee replacement" with the listed illness "Non infective arthritis" first. The 24 months and the conclusion are right, and the specified list also names **"Joint replacement surgeries"** under surgical procedures (C.1.b.vi). Open that clause if asked.
- **Step 3** next-step bullets can be phrased as instructions ("Do not admit/pay under standard cover until ..."). It is a suggestion, and the officer decides. The date and months in the table are the tool's.
